$(function(){
    ...
    function fadeOutFile( file ) {
        var $li = $('#'+file.id);
        delete percentages[ file.id ];
        updateTotalProgress();
        $li.off().find('.file-panel').off().end().fadeOut(1000);
    }
        ...
        switch ( state ) {
            ...
            case 'finish':
                ...
                for(var i=0;i<uploader.getFiles().length;i++){
                	var uploadFiles = uploader.getFiles();
                	if(uploadFiles[i].statusText==""){
                		fadeOutFile(uploadFiles[i]);
                	}
                }
                $('#filePicker2').removeClass( 'element-invisible' );
